...
import { DemoApiError, getDemoSession, loadStaffDemoState, malaysiaDateValue, type DemoState } from './demo-booking';
import { minuteValue, roomSchedule, shopTime, type RoomStatus, type RoomVisit } from './room-schedule';
import styles from './rooms.module.css';

export function RoomBoard({ state, date, time, locale, onOpenBooking }: { state: DemoState; date: string; time: string; locale: Locale; onOpenBooking?: (id: string) => void }) {
  ...
  function Visit({ visit }: { visit: RoomVisit }) { return <article className={styles.visit}><div><strong>{visit.assignment.start}–{visit.assignment.end}</strong><span>{t('Cleaning until', '清洁至')} {visit.assignment.cleanupEnd}</span></div><div><strong>{visit.guestName}</strong><span>{visit.treatment[locale]}</span><span>{visit.staffNumber} · {visit.therapist[locale]}</span><small>{visit.reference}</small></div>{onOpenBooking && <button onClick={() => onOpenBooking(visit.bookingId)}>{t('Manage booking', '管理预约')}</button>}</article>; }
