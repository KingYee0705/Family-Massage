...
export default function StaffPage() {
  ...
  const [tab, setTab] = useState<'overview' | 'earnings' | 'bookings' | 'team'>('overview');
    ...
    {loading ? <div className={styles.loading} role="status">{t('Opening staff workspace…', '正在打开员工工作台…')}</div> : !user ?
      <section className={styles.loginLayout}>
        ...
        <section className={styles.content}>
          ...
          {tab === 'earnings' ? earnings ? <TodayEarnings summary={earnings} locale={locale} canOpenBookings={user.role !== 'therapist'} onOpenBooking={(id) => open({ type: 'booking', id })} /> : <p role="status">{t('Loading today’s earnings…', '正在加载今日收入…')}</p> : !state ? <p role="status">{t('Loading bookings…', '正在加载预约…')}</p> : tab === 'team' ? <>
